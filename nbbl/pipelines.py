...
class NnbblPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('nbbl.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `nbbl`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM nbbl WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO nbbl (date, title, link, content)"
